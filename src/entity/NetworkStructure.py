...
class NetworkStructureHandler(object):
    ...
    def __init__(self, config_file='../config/config.ini'):
        ...
        try:
            ...
            # self.unit_size
            try:
                self.unit_size = float(conf.get("network", "unitSize"))
                if self.unit_size <= 0:
                    raise Exception("param network.unitSize should be positive!")
            except configparser.NoOptionError:
                logging.info("[INFO] unitSize params not detect!")
                self.unit_size = 0
            ...
            try:
                ...
                if self.unit_size != 0:
                    self.radius_params = list()
                else:
                    raise Exception("param network.unitSize and network.radiusParams should at least set one!")
            ...
            try:
                ...
                logging.info("[INFO] Curvature params not detect!")
                self.curvature = 0
    ...
    def print_config(self):
        ...
        if self.unit_size != 0:
            logging.info("单元尺寸(lc): " + str(self.unit_size))
        else:
...
class NetworkStructure(object):
    ...
    def calculate_unit_size(self):
        ...
        :return:
        """
        # E(X^3) = μ^3 + 3μσ^2, if X ~ N(μ, σ)
        pore_volumn = 4.0 / 3.0 * np.pi * ((self.nc.radius_params[0] ** 3) + 3 * self.nc.radius_params[0] * (self.nc.radius_params[1] ** 2))
    ...
    def calculate_pore_size(self):
        ...
        """
        var_ratio = 0.2
        pore_radius = np.power((self.unit_size ** 3) * self.nc.porosity * 3.0 / 4.0 / np.pi / (1 + 3 * (var_ratio ** 2)), 1.0/3.0)
        hi = pore_radius
        lo = 0
        iter_count = 0
        while np.abs(hi - lo) > 1:
            iter_count += 1
            if iter_count > 100:
                raise Exception("Solve pore size error: iteration more than 100!")
            pore_radius = (lo + hi) / 2
            pore_volumn = 4.0 / 3.0 * np.pi * (1 + 3 * (var_ratio ** 2)) * (pore_radius ** 3)
            if self.nc.curvature:
                # 使用统计结论：在合理范围内，取r=μ-0.3σ得到的结果较准
                ...
                # E(X^2) = (E(X))^2 + D(X)
                ave_sqr = np.square(self.nc.throat_params[0]) + np.square(self.nc.throat_params[1])
            throat_length = self.unit_size - 2 * pore_radius
            throat_volumn = np.pi * ave_sqr * self.nc.coor_params[0] * throat_length / 2.0
            error = (throat_volumn + pore_volumn) / np.power(self.unit_size, 3.0) - self.nc.porosity
            if error > 0:
                hi = pore_radius
            else:
                lo = pore_radius
    ...
    def calculate_final_porosity(self):
        ...
        :return:
        """
        pore_volumn = 4.0 / 3.0 * np.pi * np.average(np.power(self.radii, 3.0))
        throat_volumn = 0
        for i in range(1, self.model_size[0] - 1):
            for j in range(1, self.model_size[1] - 1):
                for k in range(1, self.model_size[2] - 1):
                    ind1 = (i, j, k)
                    for l in range(26):
                        rp = Tools.Tools.get_relative_position(l)
                        ind2 = tuple(np.add(rp, (i, j, k)))
                        throat_volumn += np.pi * (self.throatR[i, j, k, l] ** 2) \
                                         * (self.unit_size - self.radii[ind1] - self.radii[ind2]) \
                                         * self.weight[i, j, k, l]
